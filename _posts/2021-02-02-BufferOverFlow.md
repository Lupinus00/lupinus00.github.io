...
C는 하이레벨 PL 이지만 자체적으로 데이터 무결성을 검사하는 기능은 없어서 프로그래머가 직접 데이터 무결성을 검사해야 합니다. 
...
이것을 예를 들면 8바이트 크기의 버퍼에 데이터 10바이트를 넣는 것이 가능하다는 뜻 입니다.

해당 변수의 정해진 크기보다 넘어서는 크기의 값을 넣는 것을 버퍼 오버플로우(오버런)이라고 하고 

데이터 10바이트를 메모리 8바이트에 저장하려할 때 나머지 2바이트는 다른 메모리 영역을 덮어쓰게 됩니다.

이러한 버퍼 오버런이 가져오는 피해는 프로그램이 잘못된 연산으로 의도치 않게 작동하는 상황이 생기게 됩니다.
